...
if submitted:
    with st.spinner('Wait for it...'):
        extracted_values = viator_post_request(destination=destination, start_date=start_date, end_date=end_date, user_tags=user_tags, event_number=10)
        itinerary = gpt_formatting(extracted_values, start_date, end_date)
        st.text(itinerary)
